...
- **Alerts** — a small form (metric `Picker` Cost/Tokens · operator `Picker` `<` `<=` `=` `>=` `>` · numeric text field · add button) plus a live-editable list of the rules already saved. Each list row is itself editable (the same two pickers + field) with a trash button. Rules persist to `~/.cc-token-bar/prefs.json`. On every refresh `AlertNotifier.evaluate(_:)` checks each rule against **today's** cost or token total and, when the comparison is true and that rule has not already fired today, posts a banner. A per-day de-dup map (`alerts-state.json`, keyed by rule id → day) stops repeat banners; it resets when the day rolls over. Because the app runs as an accessory and `FSWatcher` keeps refreshing on hook writes, alerts fire even with the popover closed — but only while the app is running.

  **Notification delivery — `osascript`, not `UserNotifications`.** The first implementation used `UNUserNotificationCenter`. On this ad-hoc-signed SwiftPM build `requestAuthorization` returns `UNErrorDomain` code 1 with `authorizationStatus == .notDetermined` — the system never shows the prompt and `add(_:)` always fails, because Apple's notification daemon refuses to register an app without a proper Developer ID signature. Diagnosed via a debug log written from a real `open`-launched run, not the binary run directly (running the Mach-O outside the bundle is a separate, false-negative failure mode). The fix sidesteps signing entirely: `AlertNotifier.notify(_:)` shells out to `/usr/bin/osascript -e 'display notification … with title …'`, which the system already trusts. Banners are attributed to "Script Editor" but they appear reliably and need no permission prompt. The earlier de-dup-before-delivery bug (a rule marked fired even when `add(_:)` errored, permanently muting it for the day) is gone with this path; the rule is still marked optimistically but the whole delivery is fire-and-forget through a trusted helper.
